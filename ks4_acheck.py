...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 16 16:52:00 2020
...
#        zpstar  = info[info['col1'] == date+'/'+imname]['col6'][0] # number of stars used for estimating the zeropoint
        ...
        plt.scatter((mtbl['RAJ2000']-mtbl['ALPHA_J2000'])*np.abs(np.cos(mtbl['DEJ2000']))*3600, (mtbl['DEJ2000']-mtbl['DELTA_J2000'])*3600, c='blue', alpha=0.25)
...
#        plt.show()
        ...
        plt.figure(figsize=(6,6))
        plt.scatter(mtbl['X_IMAGE'], mtbl['Y_IMAGE'], c=mtbl['sep']*3600, cmap='seismic')
        cbar    = plt.colorbar()
...
#    mimtbl  = Table([malimg0, malimg1, malimg2, malimg3], names=['files', 'misalignment', 'seeing', 'stars'])
    mimtbl  = Table([malimg0, malimg1, malimg3], names=['files', 'misalignment', 'match_stars'])
...
for k in range(len(misormag)):
    ...
    #%%
for mis in misalign:
    a = ascii.read('p'+mis+'.cat')['ELONGATION']
    b = ascii.read('p'+mis+'.cat')['ELLIPTICITY']
    print(mis, np.median(a), np.median(b))
