...
imlist  = imlistsort_ks4(glob.glob('ks4.{}.*.stack.fits'.format(target)))
weight  = imlistsort_ks4(glob.glob('ks4.{}.*.weight.fits'.format(target)))
#%%
# wimg    = weight[1]
...
catname     = '{}{}{}.cat'.format(path_cat, target, band)
segname     = '{}{}{}.seg'.format(path_cat, target, band)
apername    = '{}{}{}.aper'.format(path_cat, target, band)
bkgname     = '{}{}{}.bkg'.format(path_cat, target, band)
...
prompt_opt  = ' -GAIN '+'%.2f'%(gain)+' -PIXEL_SCALE '+'%.2f'%(pixscale)+' -SEEING_FWHM %.2f'%(fwhm)
prompt_bkg  = ' -BACK_SIZE %d'%(bkgsize)
prompt_chk  = ' -CHECKIMAGE_TYPE SEGMENTATION,APERTURES,BACKGROUND -CHECKIMAGE_NAME '+segname+','+apername+','+bkgname
# if refband == 'none':
prompt      = 'sex '+inim_single+prompt_cfg+prompt_opt+prompt_cat+prompt_bkg+prompt_chk
# else:
...
reftbl  = apass_query(radec[0], radec[1], 0.2*radius)
...
zpplot(outname='{}_{}_{}.zpcal.png'.format(target, band, inmagkey), otbl=otbl, xtbl=xtbl, inmagkey=inmagkey, inmagerkey=inmagerkey,refmagkey=refmagkey, refmagerkey=refmagerkey, zp=zp, zper=zper)
...
if band == 'B':
    ...
    magerr  = sqsum(mtbl['e_Bmag'], sqsum(mtbl['MAGERR_AUTO'], zper))
...
from astropy.stats import sigma_clip
rmsmagdif   = np.sqrt(np.mean(sigma_clip(magdif_uns,5)**2))
stdmagdif   = np.std(sigma_clip(magdif_uns,5))
...
plt.scatter(mtbl[mtbl['MAG_AUTO']+zp>13]['MAG_AUTO']+zp, magdif_uns, c='crimson', marker='o', alpha=0.3)
...
plt.title('KS4 Photometry Precision Map for {} ({}) \n Median Magnitude Difference: {} ABmag \n Magnitude Difference RMSE: {} ABmag'.format(target, band, round(mediff, 3), round(rmsmagdif, 3)))
...
#plt.show()
plt.savefig('{}_{}_graph.png'.format(target, band))
plt.close()
# zp= np.mean([zp0, zp1, zp2, zp3])
# residual map (x-axis: XWIN_IMAGE, y-axis: YWIN_IMAGE)
#%%
plt.figure(figsize=(10,9))
plt.scatter(mtbl[mtbl['MAG_AUTO']+zp>13]['X_IMAGE'], mtbl[mtbl['MAG_AUTO']+zp>13]['Y_IMAGE'], marker='o', c=magdif_uns, cmap='seismic', alpha=0.7)
...
plt.axhline(12371, ls='--', color='dodgerblue', alpha=0.5)
plt.axvline(12257, ls='--', color='dodgerblue', alpha=0.5)
#plt.show()
plt.savefig('{}_{}_plot.png'.format(target, band))
plt.close()
...
# # result
# # intbls       = ascii.read(path_cat+'KS4/'+band[0]+'_'+target+'.cat') 
